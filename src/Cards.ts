...
export function isValidCard(idOfCard: number) {
  if (idOfCard == WILD.Draw4 || idOfCard == WILD.Wild) return true;
  if (idOfCard >= RED._0 && idOfCard <= RED._Draw2) return true;
  if (idOfCard >= GREEN._0 && idOfCard <= GREEN._Draw2) return true;
  if (idOfCard >= BLUE._0 && idOfCard <= BLUE._Draw2) return true;
  if (idOfCard >= YELLOW._0 && idOfCard <= YELLOW._Draw2) return true;
...
export function getCardColor(idOfCard: number) {
  if (idOfCard & 0b01000000) return 4;
  if (idOfCard & 0b00100000 && idOfCard & 0b00010000) return 3;
  if (idOfCard & 0b00100000) return 2;
  if (idOfCard & 0b00010000) return 1;
...
export function isValidStartCard(idOfCard: number) {
  return (
    (idOfCard >= RED._0 && idOfCard <= RED._9) ||
...
export function isCardPlayable(idOfCard: number, topCard: number) {
  if (idOfCard == WILD.Wild || idOfCard == WILD.Wild) {
    return true;
  }
  if (
    (idOfCard == RED._Draw2 ||
