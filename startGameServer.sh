...
for file in ./out/*.js; do
    ...
    # sed -i -E "s/(import.*PlayerWsConnection)/\1\.mjs/" WebSocketGameServer.mjs
    #fileName = eval $(basename $file .mjs)
    #echo "${fileName}"
    #if [ "${fileName}" = "WebSocketGameServer" ]; then
      # echo "$file"
    ...
    sed -i -E "s/(import.*\/WebSocketGameServer)'/\1\.mjs'/" "./out/$(basename "$file" .js).mjs"
...
node ./out/WebSocketGameServer.mjs